(function () {
   ...
   var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    // Anything already on or above the first painted screen shows at once; smooth scrolling only after the page has settled.
   items.forEach(function (el) { if (el.getBoundingClientRect().top < window.innerHeight * 1.1) el.classList.add('in'); });
    window.addEventListener('load', function () { setTimeout(function () { document.documentElement.classList.add('smooth'); }, 300); });
